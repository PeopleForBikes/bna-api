...
use lambdas::cities::mapper::post_cities_adaptor;
use tracing::info;
...
async fn function_handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    // let mut active_city: entity::city::ActiveModel = wrapper.into_active_model();
    ...
    // if fetch_country(&db, &country).await?.is_none() {
    ...
    //     match fetch_state_region_crosswalk(&db, &state_full).await? {
    //         Some(model) => {
